...
build/$(1)_image_inspect.txt: $(1)/Containerfile
	mkdir -p build
	finch build $(1) -f $$< --tag $(BASE)-$(1)
	finch inspect $(BASE)-$(1) > build/$(1)_image_inspect.txt
...
build/$(1)_run_output.txt: build/$(1)_image_inspect.txt
	mkdir -p build
	finch run $(BASE)-$(1) /usr/bin/python3 -c "import uuid; print(uuid.uuid4())" > build/$(1)_run_output.txt
